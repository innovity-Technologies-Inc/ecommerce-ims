                ...
                <form action="{{ route('admin.sections.bestsellers.update') }}" method="post">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">Bestsellers Section Settings</h4>
                        </div>
                        <div class="card-body">
                            <div class="row">
                                <div class="col-lg-6">
                                ...
                                <div class="col-lg-6">
                                    <div class="mb-3">
                                        ...
                                        <div class="form-check form-check-inline">
                                            ...
                                            <label class="form-check-label" for="mode_organic">Organic (By Sales Count)</label>
                                        </div>
                                        <div class="form-check form-check-inline">
                                        ...
                                        <select name="product_ids[]" id="product_ids" class="form-control select2_list" multiple>
                                            @foreach($products as $product)
                                                <option value="{{ $product->id }}" {{ in_array($product->id, old('product_ids', $selectedProducts->pluck('id')->toArray())) ? 'selected' : '' }}>
                                                    {{ $product->name }} (Sales: {{ $product->sales_count }})
                                                </option>
                                            @endforeach
                                        </select>
                                        <small class="text-muted">Drag and drop to reorder is not supported in this simple select, but they will be saved in the order selected.</small>
                                        @error('product_ids')
                                        <span class="small text-danger">{{$message}}</span>
                                        @enderror
                                    </div>
