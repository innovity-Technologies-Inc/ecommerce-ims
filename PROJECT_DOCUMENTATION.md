...
- **How it Works (Technical Flow):**
    ...
    7. **Reporting & Exporting (REQ-140, REQ-141, REQ-142):** 
        ...
        *   **Server Compatibility (REQ-232):** Implemented a pure PHP fallback for the `NumberFormatter` class in `HelperClass::numberToWords()`. This ensures the application remains functional on servers where the PHP `intl` extension is not enabled.
- **Data & Storage (DB Connectivity):**
...
- **Data & Storage (DB Connectivity):**
    ...
    *   `warehouse_stock_limits`: Source for localized alert thresholds.
    ...
    3. **Automated Tracking & Session Accumulation (REQ-237):** 
        - The system captures the first clock-in of the day as the primary `clock_in` time.
    ...
    *   `payslips`: Stores individual employee payouts linked to a generation batch.
    *   `general_settings`: provide the dynamic currency symbol and timezone settings.
...
- **Data & Storage (DB Connectivity):**
    ...
    - `minio_data` Docker volume → MinIO container → designated bucket → path-based file storage.
...
- **How it Works (Technical Flow):**
    ...
    5. **Client Vertical Alignment:** In the desktop navbar, the `.row` is updated with the `align-items-center` class. The `.col-md-10` container is converted to a flexbox layout using `d-flex align-items-center justify-content-between`, and top margins on `.logo` and `.main-navigation` are cleared (`m-0`). This centers both the logo and the menu links vertically and anchors them elegantly across the navbar, while `.header_account_area` retains its default positioning and absolute offsets for dropdown search widgets.
