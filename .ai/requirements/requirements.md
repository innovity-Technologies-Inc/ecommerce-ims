...
- [ ] **REQ-134:** Seeder Fixes & Product Alignment: Fix all existing seeders to ensure they run without errors (e.g., duplicate entries, missing fields) and align the `ProductSeeder` logic with the current product creation form and service.
- [ ] **REQ-135:** Product Image Upload Refinement: Update image upload logic to allow max 2MB per image, limit gallery uploads to 5 images at a time, and introduce a dedicated "Primary Image" field separate from the "Gallery Images" field.
