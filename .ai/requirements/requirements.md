...
- [x] **REQ-120:** Granular Stock Ledger Entries: Ensure that stock movements for serial-tracked items are logged as individual entries (change_qty: 1 or -1) per serial number in the `stock_ledgers` table. Added `batch_serial_id` to `stock_ledgers` and created a dedicated Stock Ledger Report view in the Admin Panel.
