...
- [x] **File:** `app/Http/Requests/Admin/ProductRequest.php`
- [x] **Change:** Refine the custom error messages in the `messages()` method.
...
- [x] **File:** `resources/views/admin/products/form.blade.php`
- [x] **Change:** Add a `<p class="text-muted extra-small mt-1">` or similar note under the "Main Description" label.
    - Text: "Note: For optimal formatting, it is recommended to copy and paste content from Google Docs or Microsoft Word." 

## Verification
- [x] Attempt to upload an image > 2MB and verify the specific error message.
- [x] Verify the guidance note is visible under the Main Description field.
